...
import { ledger, type Ledger, ProposalState } from '../../contract/src/managed/bboard/contract/index.js';
...
const displayLedgerState = async (
  ...
  if (ledgerState === null) {
    ...
    const proposalStatus = ledgerState.proposalState === ProposalState.ACTIVE ? 'active' : 'closed';
    const currentProposal = !ledgerState.proposal.is_some ? 'none' : ledgerState.proposal.value;
    logger.info(`Current proposal state is: '${proposalStatus}'`);
    logger.info(`Current proposal is: '${currentProposal}'`);
...
const displayDerivedState = (ledgerState: BBoardDerivedState | undefined, logger: Logger) => {
  if (ledgerState === undefined) {
    ...
    const proposalStatus = ledgerState.state === ProposalState.ACTIVE ? 'active' : 'closed';
    const latestMessage = ledgerState.state === ProposalState.ACTIVE ? ledgerState.message : 'none';
    logger.info(`Current proposal state is: '${proposalStatus}'`);
  ...
  try {
    while (true) {
      ...
      try {
        switch (choice) {
          case '1': {
            const proposal = await rli.question(`What proposal do you want to create? `);
            await bboardApi.createProposal(proposal);
            break;
          }
          case '2':
            await bboardApi.voteYes();
            break;
          case '3':
            await bboardApi.voteNo();
            break;
          case '4':
            await bboardApi.closeProposal();
            break;
          case '5':
...
export const run = async (config: Config, testEnv: TestEnvironment, logger: Logger): Promise<void> => {
  ...
  try {
    ...
    const zkConfigProvider = new NodeZkConfigProvider<'createProposal' | 'voteYes' | 'voteNo' | 'closeProposal'>(config.zkConfigPath);
